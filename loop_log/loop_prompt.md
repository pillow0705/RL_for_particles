...
- 禁止嵌入型（min/max < 0.414）
- ⚠️ **全部资源倾注双球实验，其他一切实验（包括 D7 长跑）全部 kill，不再监控**
- 两台服务器 S1、S2 全部用于双球 smoke 探索
...
---
## ⚠️ 重要发现：尺度不变性实验为硬编码容差伪影（loop 143-168）

**发现（loop ~168）**：之前"小粒径系统（如[0.3,0.6]）phi 高于标准粒径"的现象是**硬编码绝对容差**导致的伪影，并非真实物理规律。

**原因**：
- `collision_tol=0.05` 和 `edge_tol=0.05` 是绝对距离，不随粒子尺寸缩放
- 对于 [0.3,0.6] 系统（r_sum≈0.45），`collision_tol/r_sum≈11.1%`（允许极大相对重叠）
- 对于 B3 [0.5,1.0] 系统（r_sum≈0.75），`collision_tol/r_sum≈6.7%`（相对重叠较小）
- 小粒子系统"偷"了更多重叠空间，phi 虚高

**修复（loop 168）**：已将 `physics.py` 和 `train.py` 中的绝对容差改为相对容差：
- `physics.py`：`gap < -collision_tol * r_sum`（相对于粒子对半径和缩放）
- `train.py`：`dist <= (r1+r2) * (1 + edge_tol)`（相对缩放）

**结论**：
- loops 143-168 中 [0.3,0.6]、[0.35,0.7] 等小粒径实验结果**不可作为跨尺度比较基准**
- 同尺度（相同粒子尺寸）实验之间的相对比较仍然有效
- 修复后的代码才能做真实的尺度不变性研究
- 当前两个长跑（S1: [0.3,0.6] 200轮、S2: [0.35,0.7] 100轮）的 phi 结果受伪影影响，仅供参考
...
|------|------|--------------|------|
| [0.3,0.6]×M2×H2 | 双球（旧绝对容差，伪影） | **0.7114** | 200轮长跑进行中(iter50/200) |
| [0.35,0.7]×M2×H2 | 双球（旧绝对容差，伪影） | ~0.695 | 100轮长跑进行中(iter52/100) |
| D7×M2×H2（[0.62,1.42]9峰） | 挤压多峰 | **0.6931** | 已完成 |
| D6×M2×H2（[0.65,1.45]9峰） | 挤压多峰 | 0.6790 | 完成 |
| D1×M2×H2（[0.7,1.4]8峰） | 挤压多峰 | 0.6751 | 完成 |
| B3×M2×H2（[0.5,1.0] 正常尺度） | 双球挤压 | ~0.67-0.69 | 已有smoke结果 |
