...
当前三峰[0.5,0.9,1.4]中 0.5/1.4=0.357 属于嵌入型（<0.414）。需要专门安排挤压型（所有相邻直径比 >0.414）的实验，例如 [0.6, 0.9, 1.4]（0.6/1.4=0.429，0.6/0.9=0.667）或 [0.7, 1.0, 1.4] 等，观察挤压型配置能否同样突破0.70。
...
2. **重点探索挤压型（所有相邻直径比 > 0.414）**：目前嵌入型 [0.4,0.8,1.4] 已充分验证，挤压型方向尚未被系统性探索。需要设计多组挤压型配置（如 [0.6,0.9,1.3]、[0.65,0.95,1.4]、[0.7,1.05,1.5] 等），通过大量冒烟测试（每组 10~20 轮）寻找潜在突破点。
3. **挤压型设计原则**：确保所有相邻比值（小/大）均 > 0.414，即真正的挤压型物理机制。用不同的粒径跨度、不同峰数（双峰/三峰/四峰）系统覆盖参数空间。
4. **看到此提醒后，在每次 loop 的记录中注明"已收到探索指令"，并说明本次 loop 为挤压型探索安排了哪些冒烟测试（或说明为何本次未安排）。**
