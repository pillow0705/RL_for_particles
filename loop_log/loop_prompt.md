...
4. **重点探索 [0.7, 1.4] 区间**：用户特别指出要关注粒径在 0.7~1.4 范围内的配置。在此区间内系统设计多峰分布，例如：
   - 双峰：[0.7, 1.4]（min/max=0.500）
   - 三峰均匀：[0.7, 1.05, 1.4]（min/max=0.500）、[0.7, 1.0, 1.3]、[0.8, 1.1, 1.4]
   - 三峰偏小：[0.7, 0.9, 1.4]（min/max=0.500）、[0.7, 0.85, 1.1]
   - 四峰：[0.7, 0.9, 1.1, 1.4]（min/max=0.500）
   - 多分散：np.arange(0.7, 1.45, 0.05) 等
